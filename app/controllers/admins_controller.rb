...
class AdminsController < ApplicationController
  ...
  manage_users = [:edit_user, :promote, :demote, :ban_user, :unban_user, :approve, :reset, :merge_user, :export_users]
  ...
  def invite
    ...
    emails.each do |email|
      ...
      if phone && num_adh
        ...
        invitation = create_or_update_invite(email, phone, num_adh, smscode, current_user.email)
        send_invitation_email(current_user.name, email, invitation.invite_token)
        send_invitation_sms(current_user.name, phone, smscode)
      else
        flash[:alert] = I18n.t(params[:message], default: I18n.t("invite_input_error"))
      end
      n += 1
    end
  ...
  def export_users
    ...
    respond_to do |format|
      ...
      format.pdf do
        ...
        page_size: 'A4',
        template: "admins/export_users.html.erb",
        layout: "pdf.html"
      end
      format.xlsx do
        render xlsx: "Utilisateurs",
        template: "shared/_export_users.xlsx.axlsx"
      end
    end
  end
  ...
  def create_or_update_invite(email, phone, num_adh, smscode, adminmail)
    ...
    if invite.present?
      ...
      # Creates invite
      invite = Invitation.create(email: email, provider: @user_domain, tic_phone: phone, tic_num_adh: num_adh, tic_smscode: smscode, tic_adminmail: adminmail)
    end
