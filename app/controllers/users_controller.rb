...
class UsersController < ApplicationController
  ...
  def create
    ...
    send_registration_email(@invite.tic_adminmail)
...
#                       .shared_list_search(params[:search])
    initial_list = User.with_role(roles_can_appear)
                       .shared_list_search(params[:search])
  ...
  def user_params
    params.require(:user).permit(:name, :email, :image, :password, :password_confirmation,
      :new_password, :provider, :accepted_terms, :language, :tic_num_adh)
  end

  def send_registration_email(email)
    if invite_registration
      send_invite_user_signup_email(@user, email)
