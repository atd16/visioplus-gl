...
class SessionsController < ApplicationController
  ...
  def confirm_signup
    ...
    if params[:invitation][:user_smscode] == @invite.tic_smscode
      @invite.update(tic_is_phone_valid: true)
      redirect_to signup_path(invite_token: session[:invite_token])
      return
    end
    redirect_to confirm_sms_path(invite_token: session[:invite_token]), flash: { alert: I18n.t("signup.invalid_sms") }
