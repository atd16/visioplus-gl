...
module Populator
  ...
  def server_rooms_list
    if Rails.configuration.loadbalanced_configuration
      ...
      if !current_user.has_role?(:admin)
        Room.where(owner: current_user.id).includes(:owner).admins_search(@search).admins_order(@order_column, @order_direction, @running_room_bbb_ids)
      else
  ...
  def recordings_to_show(user = nil, room = nil)
    if user.present?
      ...
      if !current_user.has_role?(:admin)
        Room.where(owner: current_user.id).pluck(:bbb_id)
      else
        Room.pluck(:bbb_id)
      end
      latest_recordings
