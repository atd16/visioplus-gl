...
class RoomsController < ApplicationController
  ...
  before_action :find_room, except: [:create, :join_specific_room, :cant_create_rooms, :vote_infos]
  before_action :verify_room_ownership_or_admin_or_shared, only: [:start, :shared_access]
  before_action :verify_room_ownership_or_admin, only: [:update_settings, :destroy, :preupload_presentation, :remove_presentation, :votes, :signs, :participants]
  ...
  before_action :find_room_from_bbb_id, only: [:vote_infos]
  ...
  def create
    ...
    if current_user.main_room.nil?
	current_user.update_attribute(:main_room, @room)
    end
  ...
  def show
    ...
    @anyone_can_start = @room_settings["anyoneCanStart"]
    ...
    if @room_running && Rails.configuration.tic_allow_except_invite && params[:t] && @room.tic_except_join_token && params[:t] == @room.tic_except_join_token
      @valid_token = true
    end
    ...
    if current_user && (@room.owned_by?(current_user) || @shared_room)
      ...
      # user can start ? (owner or admin)
      @user_can_start = @room.owned_by?(current_user) || current_user&.admin_of?(@room.owner, "can_manage_rooms_recordings")
  ...
  def join
    return redirect_to root_path,
      flash: { alert: I18n.t("administrator.site_settings.authentication.user-info") } if auth_required && !params[@room.invite_path][:valid_token]
    ...
    # check if user has validate checkbox
    unless @room.owned_by?(current_user) || params[@room.invite_path]
        return redirect_to root_path
    end

    if !Signature.find_by(user: current_user, room: @room, disconnected_at: nil)
      Signature.create(room: @room, user: current_user)
    end
  ...
  def start
    ...
    # Generate tic_except_join_token
    if Rails.configuration.tic_allow_except_invite
      @room.tic_except_join_token = loop do
        random_token = SecureRandom.urlsafe_base64(nil, false)
        break random_token unless Room.exists?(tic_except_join_token: random_token)
      end
      logger.info "Support : room #{@room.uid} has a new exceptionnal joining token : #{@room.tic_except_join_token}"
      @room.save
    end
  ...
  def shared_access
    begin
      ...
      if params[:add]
        params[:add].each {|kp, p|
          uid_list.push(p[:uid])
          final_list.push(p)
        }
      end

      new_list = User.where(uid: uid_list).pluck(:id)
      ...
      if final_list
        final_list.each do |user|
          id = User.where(uid: user[:uid]).pluck(:id).first
          status = user[:status] ? user[:status] : "nv"
          weight = user[:weight] ? user[:weight] : 0
          proc_id = user[:proc] ? User.where(uid: user[:proc]).pluck(:id).first : nil
          if users_to_add.include?(id)
            SharedAccess.create(room_id: @room.id, user_id: id, tic_weight: weight, tic_status: status, tic_proc_user: proc_id)
          else
            SharedAccess.find_by(room_id: @room.id, user_id: id).update(tic_weight: weight, tic_status: status, tic_proc_user: proc_id)
          end
        end
      end
  ...
  def shared_users
    ...
    respond_to do |format|
      format.json { render body: @room.shared_access.joins("INNER JOIN users ON users.id = shared_accesses.user_id").select("shared_accesses.*", "users.uid", "users.name", "users.image").to_json }
    end
  end

  # GET /:room_uid/votes
  def votes
    @votes = Vote.where(room_id: @room.id, cancelled: false)
    respond_to do |format|
      format.html
      format.pdf do
        render pdf: "Votes-#{@room.id}",
        page_size: 'A4',
        template: "rooms/votes.html.erb",
        layout: "pdf.html"
      end
      format.xlsx do
        render xlsx: "Votes-#{@room.id}",
        template: "shared/_votes.xlsx.axlsx"
      end
    end
  end
  ...
  # GET /:room_uid/signs
  def signs
    @signs = Signature.where(room_id: @room.id)
    respond_to do |format|
      format.html
      format.pdf do
        render pdf: "Emargement-#{@room.id}",
        page_size: 'A4',
        template: "rooms/signs.html.erb",
        layout: "pdf.html"
      end
      format.xlsx do
        render xlsx: "Emargement-#{@room.id}",
        template: "shared/_signs.xlsx.axlsx"
      end
    end
  end

  # GET /:room_uid/participants
  def participants
    @participants = @room.shared_access
    respond_to do |format|
      format.html
      format.pdf do
        render pdf: "Participants-#{@room.id}",
        page_size: 'A4',
        template: "rooms/participants.html.erb",
        layout: "pdf.html"
      end
      format.xlsx do
        render xlsx: "Participants-#{@room.id}",
        template: "shared/_participants.xlsx.axlsx"
      end
    end
  end

  # GET /:bbb_id/vote_infos
  def vote_infos
    respond_to do |format|
    format.json { render body: @room.shared_access.joins("INNER JOIN users ON users.id = shared_accesses.user_id LEFT JOIN users AS proxy ON proxy.id = shared_accesses.tic_proc_user").where("users.uid=? OR proxy.uid=?", params[:user_uid], params[:user_uid]).select("shared_accesses.id", "shared_accesses.tic_weight as weight", "shared_accesses.tic_status as status", "users.uid as voting_uid", "users.name as voting_name", "proxy.uid as proxy_uid", "proxy.name as proxy_name").to_json }
    end
  end
  ...
  # Find the room from bbb id
  def find_room_from_bbb_id
    @room = Room.includes(:owner).find_by!(bbb_id: params[:bbb_id])
  end
